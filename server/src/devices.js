// Device registry — relational metadata for each tracked unit.
// In the full dual-database design this is the PostgreSQL `motorcycles` table
// (joined with `rentals` for the active renter). Live position/speed/battery do
// NOT come from here — they arrive as real telemetry from the GPS devices.
//
// `pos` is only a last-known seed so a freshly-opened dashboard can place the
// unit on the map before its first packet arrives; `online` flips to true on
// the first real telemetry message.
